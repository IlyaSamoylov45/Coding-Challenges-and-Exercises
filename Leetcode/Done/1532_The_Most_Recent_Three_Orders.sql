...
FROM (SELECT name, customer_id, order_id, order_date, ROW_NUMBER() OVER(PARTITION BY customer_id ORDER BY order_date DESC) AS three
    FROM Customers JOIN Orders USING(customer_id)
) x
WHERE x.three IN (1,2,3)
...
FROM (
    SELECT name,
           ...
           order_id,
           order_date,
           ROW_NUMBER() OVER(PARTITION BY customer_id ORDER BY order_date DESC) AS three
    FROM Customers JOIN Orders USING(customer_id)
    ) x
WHERE x.three IN (1,2,3)
ORDER BY customer_name, customer_id, order_date DESC
