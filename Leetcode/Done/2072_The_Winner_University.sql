...
SELECT CASE
        WHEN nys.excellent_students > cas.excellent_students THEN 'New York University'
        WHEN nys.excellent_students < cas.excellent_students THEN 'California University'
        ELSE 'No Winner'
       END
       AS winner
FROM (SELECT COUNT(*) AS excellent_students
      FROM NewYork
      WHERE score >= 90) AS nys,
      (SELECT COUNT(*) AS excellent_students
       FROM California
       WHERE score >= 90) AS cas
